...
var $author$project$Date$isLeapYear = function (year) {
	return (!(year % 4)) && ((!(!(year % 100))) || (!(year % 400)));
};
var $author$project$Date$numberOfDays = function (d) {
	var _v0 = _Utils_Tuple2(
		d.month,
		$author$project$Date$isLeapYear(d.year));
	switch (_v0.a) {
		case 1:
		...
		default:
			return $elm$core$Result$Err(
				'Invalid month number: ' + $elm$core$String$fromInt(d.month));
	}
};
var $author$project$Date$date = function (d) {
	var build = function (maxNumberOfDays) {
		return ((d.day < 1) || (_Utils_cmp(d.day, maxNumberOfDays) > 0)) ? $elm$core$Result$Err(
			'Invalid day number: ' + $elm$core$String$fromInt(d.day)) : $elm$core$Result$Ok(
	...
	return A2(
		...
		build,
		$author$project$Date$numberOfDays(d));
};
var $author$project$Main$init = _List_fromArray(
	[
		{
		amount: 150,
		business: {id: '201637691', name: 'אדווה דולב'},
		date: $author$project$Date$date(
			{day: 25, month: 9, year: 2022}),
		description: 'שיעור',
		num: 40001
	},
		{
...
var $author$project$Date$toDataString = function (d) {
	var day = d.a.day;
	var month = d.a.month;
	var year = d.a.year;
	return A2(
...
var $author$project$Date$toShortString = function (d) {
	var day = d.a.day;
	var month = d.a.month;
	var year = d.a.year;
	return A2(
...
var $author$project$Main$viewDate = function (rd) {
	if (rd.$ === 'Ok') {
		var d = rd.a;
		return A2(
			$elm$html$Html$time,
			_List_fromArray(
				[
					...
					$author$project$Date$toDataString(d))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(
					$author$project$Date$toShortString(d))
				]));
	} else {
		return A2(
			$elm$html$Html$span,
			...
			_List_fromArray(
				[
					$elm$html$Html$text('INVALID DATE')
				]));
	}
};
var $author$project$Main$view = function (model) {
	var invoiceRow = function (invoice) {
		return A2(
			$elm$html$Html$tr,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							'#' + $elm$core$String$fromInt(invoice.num))
						])),
					A2(
					$elm$html$Html$td,
					_List_Nil,
					_List_fromArray(
						[
							$author$project$Main$viewDate(invoice.date)
						])),
					A2(
					$elm$html$Html$td,
					_List_Nil,
					_List_fromArray(
						[
							$elm$html$Html$text(invoice.description)
						])),
					A2(
					$elm$html$Html$td,
					_List_Nil,
					_List_fromArray(
						[
							$elm$html$Html$text(
							$elm$core$String$fromFloat(invoice.amount) + '₪')
						])),
					A2(
					$elm$html$Html$td,
					_List_Nil,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('קבלות')
					])),
				A2(
				$elm$html$Html$table,
				_List_Nil,
				A2($elm$core$List$map, invoiceRow, model))
			]));
};
